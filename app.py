...
header[data-testid="stHeader"] { background-color: #0B0F0E; border-bottom: 1px solid rgba(46,204,113,0.08); }
...
# ── Top navigation bar (always rendered; doubles as mobile fallback) ──────────
_NAV_LABELS = ["The Menu", "Recommender", "Walkthrough", "Dashboard", "Compare"]

# Inject a uniquely-IDed anchor before each button so CSS :has() can target it
_nav_cols = st.columns(len(_NAV_LABELS))
for _idx, (_label, _col) in enumerate(zip(_NAV_LABELS, _nav_cols)):
    with _col:
        _active = _label == page
        # Inject an anchor with a known id immediately before this button's column slot
        st.markdown(f'<span id="tnav-{_idx}"></span>', unsafe_allow_html=True)
        if st.button(_label, key=f"topnav_{_label}", use_container_width=True):
            st.session_state["_nav_index"] = _idx
            st.rerun()

# CSS: style all topnav buttons uniformly, then highlight the active column's button.
# We target via the column's nth-of-type position, which is stable across Streamlit versions.
_active_idx = _NAV_LABELS.index(page)  # 0-based
_col_sel_base = "div[data-testid='stHorizontalBlock'] > div[data-testid='column']"
_inactive_btn = f"{_col_sel_base} .stButton > button"
_active_btn   = f"{_col_sel_base}:nth-child({_active_idx + 1}) .stButton > button"
st.markdown(f"""<style>
{_inactive_btn} {{
    font-size:0.75rem!important;padding:0.4rem 0.3rem!important;border-radius:9px!important;
    letter-spacing:0.02em!important;background:rgba(20,30,25,0.6)!important;
    border:1px solid rgba(46,204,113,0.15)!important;color:#8A9B90!important;
    box-shadow:none!important;transform:none!important;
}}
{_inactive_btn}:hover {{
    background:rgba(46,204,113,0.08)!important;border-color:rgba(46,204,113,0.4)!important;
    color:#F0F5F2!important;box-shadow:none!important;transform:none!important;
}}
{_active_btn} {{
    background:rgba(46,204,113,0.14)!important;border:1px solid rgba(46,204,113,0.55)!important;
    color:#2ECC71!important;font-weight:700!important;box-shadow:none!important;
}}
</style>""", unsafe_allow_html=True)

st.markdown('<div style="border-bottom:1px solid rgba(46,204,113,0.1);margin:0 0 1.5rem;"></div>', unsafe_allow_html=True)
