...
def card(inv, clickable=False, selected=False):
    ...
    style = f"background:rgba(20,30,25,0.7);border:1px solid {border};border-radius:16px;padding:1.4rem 1.3rem;margin-bottom:0.6rem;transition:all 0.3s ease;{glow}backdrop-filter:blur(8px);"
    return f"""
    <div style="{style}">
...
with st.sidebar:
    ...
    _NAV_OPTIONS = ["📋  The Menu", "🎯  Recommender", "📖  Walkthrough", "📊  Dashboard", "⚖️  Compare"]
    ...
    for idx, inv in enumerate(filtered):
        with cols[idx % 3]:
            ...
            if st.button(f"View Guide →", key=f"menu_btn_{inv['name']}"):
                ...
                st.session_state["_nav_index"] = 2  # Walkthrough is index 2
                st.rerun()
